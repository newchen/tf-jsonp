...
function getJSONPCallbackName() {
    return 'jsonpCallback_' + new Date().getTime() + '_' + Math.random().toString().substr(2);
}

function JSONP(url) {
    var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    var qsConfig = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};

    if (!url) return;

    var callbackName = getJSONPCallbackName();

    var _options$callback = options.callback,
        callback = _options$callback === undefined ? 'callback' : _options$callback,
        _options$timeout = options.timeout,
        timeout = _options$timeout === undefined ? 10000 : _options$timeout,
        opts = _objectWithoutProperties(options, ['callback', 'timeout']);

    var params = _extends(_defineProperty({}, callback, callbackName), opts);

    var mark = url.indexOf('?') === -1 ? '?' : '&';
    url = url + mark + _qs2.default.stringify(params, qsConfig);
    ...
    var absortPromise = new Promise(function (resolve, reject) {
